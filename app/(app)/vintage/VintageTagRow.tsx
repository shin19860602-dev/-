...
import { setVintageTagActive, updateVintageTag, deleteVintageTag, moveVintageTag } from "./actions";

export default function VintageTagRow({
  id,
  name,
  active,
  canEdit,
    ...
    return (
      <form
        ...
        action={(formData) => {
          setError(null);
          startSave(async () => {
            const result = await updateVintageTag(formData);
            if (result.ok) {
              setEditing(false);
              router.refresh();
            } else {
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button
            ...
            onClick={() => {
              startTransition(async () => {
                await setVintageTagActive(id, !active);
                router.refresh();
              });
            }}
          >
          ...
          <button
            ...
            onClick={() => {
              ...
              startDelete(async () => {
                const result = await deleteVintageTag(id);
                if (result.ok) {
                  router.refresh();
                } else {
                  alert(result.error);
